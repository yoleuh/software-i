...
public final class CryptoUtilities {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        n2.divide(two);
        //check remainder after dividing by two
        if (n2.isZero()) {
            //remainder = 0, n = even
            even = true;
        }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        //call to check if p is even
        boolean even = isEven(p);
        if (p.compareTo(one) > 0) {
            if (even) {
                // p/2 for smaller problem
                p.divide(two);
                //recursion
                powerMod(n, p, m);
                n3.copyFrom(n);
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            int i = 0;
            while (i < 10) {
                NaturalNumber guess = new NaturalNumber2(1);
                NaturalNumber one = new NaturalNumber2(1);
                //generate random number until guess is more than one and less than n-1
                while (guess.compareTo(one) <= 0 || guess.compareTo(n) >= 0) {
                    guess = randomNumber(n);
                }
