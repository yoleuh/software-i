...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        // find the substring of str1 that doesn't contain the overlap and add to str2
        String one = str1.substring(0, str1.length() - overlap);
        //add str2
        String combined = one + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //check precondition
        Set<String> pre = new Set2<>();
        pre.transferFrom(strSet);
        while (pre.size() > 0) {
            String x = pre.removeAny();
            strSet.add(x);
            assert !pre.contains(
                    x) : "Violation of CONTAINS_NO_SUBSTRING_PAIRS(strSet)";
        }

        //boolean and check if str is a substring of any string in strSet
        boolean sub = false;
        for (String s : strSet) {
            if (s.indexOf(str) != -1) {
                sub = true;
        ...
        //add to strSet if it is not
        if (!sub) {
            strSet.add(str);
            //remove any string in strSet that is a substring of str by moving
            //strings that are not a substring into a temp set
            Set<String> temp = new Set2<>();
            for (String s : strSet) {
                if (str.indexOf(s) == -1 || s.equals(str)) {
                    temp.add(s);
                }

            }
            //transfer back to orginal set
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        strSet.add(str1);
        //add next line to set if it is not empty
        while (!input.atEOS()) {
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int n = 0; n < text.length(); n++) {
            //print each character of the text
            if (!((text.charAt(n)) == '~')) {
                out.print(text.charAt(n));
                //print a new line on every ~ character
            } else {
                out.println();
            }
        }
